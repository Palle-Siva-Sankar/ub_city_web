        ...
        <div className="relative z-10 max-w-[1400px] mx-auto text-center px-6">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.5 }}>
            ...
            <div className="inline-flex items-center gap-4 px-8 py-3 rounded-full glass-pane border border-accent/40 mb-10 mx-auto group hover:bg-accent hover:text-black transition-all shadow-gold">
              ...
              <span className="text-[10px] font-black tracking-[0.6em] uppercase text-accent group-hover:text-black transition-colors">Global Headquarters</span>
            </div>
      ...
      <section className="py-40 px-6 md:px-12 bg-page relative z-10 transition-colors duration-500">
        <div className="max-w-[1400px] mx-auto grid lg:grid-cols-2 gap-20">
          ...
          <div className="grid sm:grid-cols-2 gap-10">
            {[
              { icon: MapPin, label: "Registry Location", value: CONTACT.address },
              { icon: Phone, label: "Direct Comms", value: CONTACT.phone },
              { icon: Mail, label: "Secure Channel", value: CONTACT.email },
              { icon: Clock, label: "Operational Window", value: CONTACT.hours },
            ].map((item, i) => (
              <motion.div
                key={i}
          ...
          >
            <iframe
              ...
              className="dark:invert dark:grayscale dark:contrast-125 transition-all duration-1000 group-hover:grayscale-0 group-hover:invert-0 group-hover:contrast-100 duration-1000"
              allowFullScreen
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
              title="Mall Location"
            />
            <div className="absolute inset-0 pointer-events-none border-[20px] border-[var(--page-bg)]/20" />
            <div className="absolute top-10 left-10 glass-pane p-6 rounded-3xl border border-accent/30 pointer-events-none shadow-gold">
               <p className="text-accent text-[10px] font-black tracking-[0.4em] uppercase">Tactical Map Overlay</p>
            </div>
          </motion.div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="text-center mb-24">
               <div className="flex items-center justify-center gap-6 mb-8">
                  ...
                  <p className="text-accent text-[11px] font-black tracking-[0.8em] uppercase">Protocol: Inquiry</p>
                  <div className="w-16 h-px bg-accent" />
               </div>
               <h2 className="text-6xl md:text-[8rem] font-black font-['Outfit'] text-ink-gradient uppercase tracking-tighter leading-[0.9] mb-8">Initiate <br/><span className="text-gradient">Dialogue.</span></h2>
               <p className="text-xl md:text-2xl text-[color:var(--text-dim)] font-medium italic border-l-2 border-accent/30 pl-8 inline-block">A dedicated concierge curator will assist you shortly.</p>
            </div>
            ...
            <div className="glass-pane lighting-card rounded-[5rem] p-12 md:p-24 border border-[var(--border)] shadow-2xl relative overflow-hidden">
              ...
              {submitted ? (
                <div className="text-center py-24">
                  ...
                  <h3 className="text-5xl font-black font-['Outfit'] text-ink-gradient mb-6 uppercase tracking-tighter">Transmission Successful</h3>
                  <p className="text-2xl text-[color:var(--text-dim)] font-medium italic">Your data signature has been received. We'll respond within 24 hours.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="grid gap-12 relative z-10">
                  <div className="grid md:grid-cols-2 gap-12">
                    <div className="space-y-6">
                      <label className="text-[11px] font-black tracking-[0.4em] uppercase text-accent ml-8">Subject Identifier</label>
                      <input required type="text" placeholder="Full Registry Name" className="w-full glass-pane border border-[var(--border)] text-ink-gradient rounded-[2.5rem] px-12 py-8 focus:outline-none focus:border-accent transition-all duration-500 font-black text-sm placeholder:opacity-20 hover:border-accent/40" />
                    </div>
                    <div className="space-y-6">
                      <label className="text-[11px] font-black tracking-[0.4em] uppercase text-accent ml-8">Digital Protocol (E-Mail)</label>
                      <input required type="email" placeholder="secure@connection.com" className="w-full glass-pane border border-[var(--border)] text-ink-gradient rounded-[2.5rem] px-12 py-8 focus:outline-none focus:border-accent transition-all duration-500 font-black text-sm placeholder:opacity-20 hover:border-accent/40" />
                    </div>
                  </div>
                  <div className="space-y-6">
                    <label className="text-[11px] font-black tracking-[0.4em] uppercase text-accent ml-8">Channel Subject</label>
                  ...
                  <div className="space-y-6">
                    <label className="text-[11px] font-black tracking-[0.4em] uppercase text-accent ml-8">Message Payload</label>
                    <textarea required rows={6} placeholder="How may we facilitate your experience?" className="w-full glass-pane border border-[var(--border)] text-ink-gradient rounded-[4rem] px-12 py-10 focus:outline-none focus:border-accent transition-all duration-500 resize-none font-medium italic text-xl leading-relaxed placeholder:opacity-20 hover:border-accent/40" />
                  </div>
                  <button type="submit" className="btn-luxe w-full mt-10 py-10 text-sm shadow-gold group">
                    <Send className="w-6 h-6 mr-4 group-hover:translate-x-2 transition-transform" /> Dispatch Transmission
                  </button>
                </form>
              )}
            </div>
