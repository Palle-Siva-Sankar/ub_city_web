...
export function CinemaMovieDetail() {
    ...
    return (
      <div className="page-wrapper bg-page min-h-screen pt-36 px-6">
        <div className="max-w-4xl mx-auto text-center py-24">
          <h1 className="text-5xl font-black font-['Outfit'] text-ink-gradient">
            Movie Not Found
          </h1>
          <Link to="/cinema" className="btn-luxe mt-8">
            Back to Cinema
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="page-wrapper bg-page min-h-screen transition-colors duration-500 pb-40">
      <section className="px-6 md:px-12 pt-12 mb-10">
        <div className="max-w-[1400px] mx-auto rounded-[4rem] overflow-hidden relative h-[450px] shadow-gold lighting-card">
          ...
          <div className="absolute inset-0 p-10 md:p-20 flex flex-col justify-end relative z-10">
            <Link
              to="/cinema"
              className="group inline-flex items-center gap-4 px-6 py-3 glass-pane border border-accent/30 rounded-full text-[9px] font-black uppercase tracking-[0.4em] text-accent hover:bg-accent hover:text-on-accent transition-all shadow-gold mb-10 w-fit"
            >
              <ArrowLeft className="w-4 h-4 group-hover:-translate-x-2 transition-transform" />{" "}
              Return to Cinematic Vault
            </Link>
            ...
            <div className="flex flex-wrap gap-4 text-[10px] font-black tracking-widest uppercase items-center">
              <span className="px-6 py-2 rounded-full glass-pane border border-white/10 text-white/90 shadow-sm">
                {movie.rating}
              </span>
              <span className="px-6 py-2 rounded-full glass-pane border border-white/10 text-white/90 shadow-sm">
                {movie.duration}
      ...
      <section className="px-6 md:px-12 mb-16">
        <div className="max-w-[1400px] mx-auto grid lg:grid-cols-[1.3fr_0.7fr] gap-12">
          <div className="glass-pane lighting-card rounded-[3.5rem] p-12 md:p-16 border border-[var(--border)] shadow-sm">
            <p className="text-ink-gradient text-2xl md:text-3xl font-light italic leading-relaxed mb-12">
              "{movie.synopsis}"
            </p>
            <div className="grid sm:grid-cols-3 gap-8 pt-10 border-t border-[var(--border)]">
              {[
                ...
                { icon: Clock, label: "Total Runtime", value: movie.duration },
                {
                  icon: Ticket,
                  label: "Price Scale",
                  value: `${formatINR(movie.priceInr)} onwards`,
                },
              ].map((info, i) => (
                <div key={i} className="space-y-3">
                  ...
                  <p className="text-ink-gradient font-black text-lg tracking-tight uppercase leading-none">
                    {info.value}
                  </p>
                </div>
              ))}
          ...
          <div className="glass-pane lighting-card rounded-[3.5rem] p-12 border border-[var(--border)] flex flex-col shadow-sm">
            <div className="flex items-center gap-4 mb-10">
              ...
              <h3 className="text-3xl font-black font-['Outfit'] text-ink-gradient uppercase tracking-tighter">
                Showtimes
              </h3>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              {movie.showtimes.map((time) => (
                <button
                  ...
                  className={`px-6 py-5 rounded-2xl text-[11px] font-black tracking-[0.2em] uppercase transition-all text-center shadow-sm active:scale-95 border ${selectedTime === time ? "bg-accent text-on-accent border-accent" : "glass-pane border-[var(--border)] hover:border-accent text-ink-gradient"}`}
                >
                  {time}
                </button>
              ))}
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-accent mt-6 opacity-60 text-center">
              Select time to deploy seat map
            </p>
          </div>
        </div>
      </section>
          ...
          >
            <div className="max-w-[1400px] mx-auto glass-pane lighting-card rounded-[4rem] p-12 md:p-20 border border-accent/20 shadow-gold">
              <div className="text-center mb-20">
                <p className="text-accent text-[11px] font-black uppercase tracking-[0.8em] mb-4">
                  Seat Selection
                </p>
                <h2 className="text-4xl md:text-6xl font-black font-['Outfit'] text-ink-gradient uppercase tracking-tighter leading-none mb-10">
                ...
                <p className="text-[9px] font-black uppercase tracking-[0.4em] text-accent opacity-40">
                  Cinema Screen
                </p>
              </div>
              ...
              <div className="flex flex-col items-center gap-10 mb-20">
                ...
                <div className="w-full">
                  <p className="text-center text-[10px] font-black uppercase tracking-[0.6em] text-accent-gold mb-8 italic">
                    Platinum Experience (INR 650)
                  </p>
                  <div className="flex flex-col items-center gap-4">
                    {["F", "E"].map((row) => (
                      <div key={row} className="flex gap-4">
                        <span className="w-8 text-[11px] font-black text-accent/40 flex items-center justify-center pt-1">
                          {row}
                        </span>
                ...
                {/* GOLD SECTION (Front) */}
                <div className="w-full">
                  <p className="text-center text-[10px] font-black uppercase tracking-[0.6em] text-accent mb-8 italic">
                    Gold Comfort (INR 450)
                  </p>
                  <div className="flex flex-col items-center gap-4">
                    {["D", "C", "B", "A"].map((row) => (
                      <div key={row} className="flex gap-4">
                          ...
                          {Array.from({ length: seatsPerRow }).map((_, i) => {
                            ...
                            return (
                              <button
                                ...
                                disabled={isReserved}
                                onClick={() => toggleSeat(seatId)}
                                className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center transition-all ${isReserved ? "bg-accent/5 text-accent/10 border border-transparent cursor-not-allowed" : isSelected ? "bg-accent text-on-accent scale-110 shadow-gold" : "glass-pane border-[var(--border)] text-accent/40 hover:border-accent hover:text-accent"}`}
                              >
                                <Armchair
                                  className={`w-4 h-4 md:w-5 md:h-5 ${isSelected ? "text-on-accent" : ""}`}
                                />
              ...
              <div className="flex flex-col lg:flex-row items-center justify-between gap-12 pt-16 border-t border-[var(--border)]">
                ...
                <div className="flex flex-col md:flex-row items-center gap-10">
                  <div className="text-right">
                    <p className="text-[9px] font-black uppercase tracking-widest text-accent mb-1">
                      Seats Selected
                    </p>
                    ...
                    <p className="text-2xl md:text-4xl font-black text-ink-gradient uppercase tracking-tighter font-['Outfit']">
                      {formatINR(
                        selectedSeats.reduce(
                          (acc, seat) =>
                            acc + (["F", "E"].includes(seat[0]) ? 650 : 450),
                          0,
                        ),
                      )}
                    </p>
        ...
        {bookingConfirmed && (
          ...
          >
            <div className="max-w-4xl mx-auto glass-pane rounded-[4rem] p-20 text-center border border-accent relative overflow-hidden shadow-gold">
              ...
              <p className="text-accent text-[11px] font-black uppercase tracking-[0.5em] mb-6">
                Booking Confirmed
              </p>
              <h2 className="text-5xl md:text-7xl font-black font-['Outfit'] text-ink-gradient uppercase tracking-tighter leading-none mb-10">
                Enjoy the <br />
              ...
              <div className="glass-pane p-10 rounded-[2.5rem] border border-accent/10 mb-12 text-left max-w-xl mx-auto">
                <p className="text-[9px] font-black uppercase tracking-[0.4em] text-accent mb-6 italic">
                  Your Ticket
                </p>
                <div className="space-y-4">
                  <div className="flex justify-between border-b border-[var(--border)] pb-4">
                    ...
                    <span className="text-[11px] font-black text-ink-gradient uppercase">
                      {movie.title}
                    </span>
                  </div>
                  <div className="flex justify-between border-b border-[var(--border)] pb-4">
                    ...
                    <span className="text-3xl font-black text-ink-gradient">
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-6">
                <Link to="/cinema" className="btn-luxe !px-16">
                  Back to Cinema
                </Link>
                <Link
                  to="/profile"
                  className="px-12 py-5 glass-pane border border-[var(--border)] text-[10px] font-black uppercase tracking-widest text-ink-gradient rounded-full hover:border-accent transition-all"
                >
                  My Bookings
                </Link>
              </div>
