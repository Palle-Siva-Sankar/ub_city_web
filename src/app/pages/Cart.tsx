...
export function Cart() {
  ...
  return (
    <div className="page-wrapper bg-page min-h-screen transition-colors duration-500 pt-12 md:pt-20">
        ...
        <div className="flex flex-col lg:flex-row gap-16 md:gap-24">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-6 mb-16">
              ...
              <div>
                <h1 className="text-4xl md:text-6xl font-black font-['Outfit'] text-ink-gradient uppercase tracking-tighter leading-none mb-2">
                  Collection
                </h1>
            ...
            <div className="space-y-8">
              <AnimatePresence mode="popLayout">
                {cart.map((item) => (
                  ...
                  >
                    <Link
                      to={`/shopping/${item.storeSlug}`}
                      className="w-32 h-32 md:w-44 md:h-44 rounded-[2.5rem] overflow-hidden bg-accent/5 border border-accent/10 shrink-0 block hover:scale-105 transition-transform duration-500"
                    >
                      <img
                        src={item.image}
                    ...
                    <div className="flex-1 text-center md:text-left">
                      ...
                      >
                        <p className="text-accent text-[8px] font-black uppercase tracking-[0.6em] mb-3">
                          {item.storeSlug}
                        </p>
                        <h3 className="text-2xl font-black font-['Outfit'] text-ink-gradient uppercase tracking-tighter mb-6 leading-none group-hover/link:text-accent transition-colors">
                          {item.name}
                        </h3>
                      </Link>
                      ...
                      <div className="flex flex-wrap items-center justify-center md:justify-start gap-8">
                        <div className="flex items-center glass-pane border border-[var(--border)] rounded-full p-1.5 bg-page/40">
                          <button
                            ...
                            className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-accent hover:text-on-accent transition-all font-black text-lg"
                          >
                            <Minus className="w-4 h-4" />
                          </button>
                          <span className="w-12 text-center text-xl font-black text-ink-gradient">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => updateQuantity(item.id, 1)}
                            className="w-12 h-12 rounded-full flex items-center justify-center hover:bg-accent hover:text-on-accent transition-all font-black text-lg"
                          >
                            <Plus className="w-5 h-5" />
                          </button>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[9px] font-black tracking-widest text-accent uppercase mb-1">
                            Valuation
                          </span>
              ...
              {cart.length === 0 && (
                <div className="py-32 text-center glass-pane rounded-[4rem] border border-[var(--border)] border-dashed border-2">
                  ...
                  <Link
                    to="/shopping"
                    className="btn-luxe py-5 px-12 inline-flex"
                  >
                    Explore Catalog <ArrowRight className="w-5 h-5 ml-4" />
                  </Link>
                </div>
              )}
            </div>
          ...
          <div className="lg:w-[480px]">
            <div className="sticky top-40 glass-pane lighting-card rounded-[4rem] border border-[var(--border)] p-12 md:p-16 overflow-hidden shadow-2xl">
              ...
              <h2 className="text-3xl font-black font-['Outfit'] text-ink-gradient uppercase tracking-tighter mb-12 pb-8 border-b border-[var(--border)] leading-none text-center">
                Summary
              </h2>
              ...
              <div className="space-y-8 mb-12">
                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.4em] text-[color:var(--text-dim)]">
                  ...
                  <span className="text-xl font-black text-ink-gradient">
                    {formatUSD(total)}
                  </span>
                </div>
                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.4em] text-[color:var(--text-dim)]">
                ...
                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.4em] text-[color:var(--text-dim)]">
                  ...
                  <span className="text-ink-gradient font-black">
                    {formatUSD(total * 0.08)}
                  </span>
                </div>
              </div>

              <div className="pt-10 border-t border-accent/20 flex justify-between items-center">
                <p className="text-[11px] font-black uppercase tracking-[0.6em] text-accent">
                  Total Valuation
                </p>
                <p className="text-5xl font-black font-['Outfit'] text-accent tracking-tighter">
              ...
              <div className="mt-12 rounded-[2.5rem] border border-[var(--border)] bg-page-bg-alt p-8">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <Link
                    to="/profile#orders"
                    className="text-[10px] font-black uppercase tracking-widest text-ink-gradient hover:text-accent transition-colors"
                  >
                    Archives
                  </Link>
                </div>
                {recentOrders.length > 0 ? (
                  <div className="space-y-4">
                    {recentOrders.map((order) => (
                      <div
                        key={order.id}
                        className="glass-pane rounded-2xl border border-[var(--border)] p-4 hover:border-accent transition-all"
                      >
                        <p className="text-[10px] font-black text-ink-gradient break-all mb-1">
                          {order.id}
                        </p>
                  ...
                  <p className="text-xs text-[color:var(--text-dim)] italic font-medium">
              ...
              <div className="mt-12 pt-10 border-t border-[var(--border)] text-center">
                <div className="inline-flex items-center gap-3 text-[9px] font-black uppercase tracking-[0.6em] text-accent/40">
                  ...
                  Mall of America Elite Protocol
                </div>
              </div>
            </div>
          </div>
