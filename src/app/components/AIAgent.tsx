...
import { Sparkles, X, MessageSquare, Send, Store, Utensils, Building2, Package, ArrowRight } from "lucide-react";
...
const SITUATIONS = [
    { id: 'brands', label: "Find Brands", prompt: "Can you help me find a specific brand?" },
    { id: 'dining', label: "Book a Table", prompt: "I'd like to book a table at a premium restaurant." },
    { id: 'leasing', label: "Contact Leasing", prompt: "I need to contact the leasing office for business inquiries." },
    { id: 'orders', label: "Track Orders", prompt: "How do I track my recent orders?" },
];

const AGENT_RESPONSES: Record<string, Message> = {
    'brands': { id: '', role: 'agent', text: 'Our directories span multiple floors featuring the world\'s most prestigious luxury brands. You can browse the interactive directory here:', action: { label: 'Explore Directory', to: '/shopping', icon: Store } },
    'dining': { id: '', role: 'agent', text: 'UB City hosts award-winning gastronomy experiences, spanning Michelin-starred chefs to exquisite cafes. Where would you like to reserve?', action: { label: 'View Restaurants', to: '/dine', icon: Utensils } },
    'leasing': { id: '', role: 'agent', text: 'Our commercial towers offer prime Grade-A corporate office spaces. I can direct you to our leasing specialists for VIP tours.', action: { label: 'Leasing Portal', to: '/reach-us', icon: Building2 } },
    'orders': { id: '', role: 'agent', text: 'To ensure maximum privacy and security, you can track all authenticated purchases and deliveries directly in your client portal.', action: { label: 'Track Orders', to: '/profile', icon: Package } }
};
...
export function AIAgent() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        {
            id: "msg-0",
            role: "agent",
            text: "Welcome to UB City. I am your exclusive digital concierge. Ask me anything about our premium brands, reservations, or strategic leasing."
        }
    ]);
    ...
    const handleSituationalPrompt = (situation: typeof SITUATIONS[0]) => {
        ...
        const userMsg: Message = { id: Date.now().toString(), role: "user", text: situation.prompt };
        setMessages(prev => [...prev, userMsg]);
        setIsTyping(true);

        setTimeout(() => {
            const botResponse = { ...AGENT_RESPONSES[situation.id], id: (Date.now() + 1).toString() };
                        ...
                        >
                            ...
                            <div className="relative z-10 p-5 md:p-6 pt-6 md:pt-6 border-b border-[var(--border)] bg-page/90 backdrop-blur-xl flex items-center justify-between touch-none">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h2 className="text-lg md:text-xl font-black font-['Outfit'] text-ink-gradient leading-none uppercase tracking-tight">AI Concierge</h2>
                                        <p className="text-[7.5px] font-bold text-accent tracking-[0.4em] uppercase mt-1 flex items-center gap-1.5 opacity-90">
                                            <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" /> Active Live
                                        </p>
                                    </div>
                                </div>
                                <button 
                                    onClick={() => setIsOpen(false)}
                                    className="w-10 h-10 rounded-full glass-pane border border-[var(--border)] flex items-center justify-center hover:bg-accent hover:text-black transition-all active:scale-90"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                                    ...
                                    {messages.map((msg) => (
                                        ...
                                        >
                                            <div className={`p-4 rounded-[1.5rem] ${
                                                msg.role === "user" 
                                                    ? "bg-accent text-black rounded-tr-[0.25rem] shadow-gold font-medium" 
                                                    : "glass-pane border border-[var(--border)] rounded-tl-[0.25rem] text-[var(--text-main)] shadow-xl"
                                                }`}
                                            >
                                                {msg.role === "agent" && (
                                                    <div className="flex items-center gap-2 mb-2 pb-2 border-b border-black/10 dark:border-white/10">
                                                        <Sparkles className={`w-3 h-3 ${msg.role === 'user' ? 'text-black' : 'text-accent'}`} />
                                                        <span className="text-[8px] font-black uppercase tracking-[0.2em] opacity-80">Concierge</span>
                                                    </div>
                                                )}
                                                <p className="text-[12px] md:text-[13px] leading-relaxed font-['Inter']">{msg.text}</p>
                                            </div>
                                            ...
                                            {msg.action && (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-[10px] font-black text-ink-gradient uppercase tracking-widest leading-none">{msg.action.label}</span>
                                ...
                                >
                                    {SITUATIONS.map(sit => (
                                        <button
                                            ...
                                            className="whitespace-nowrap shrink-0 px-4 py-2 rounded-full glass-pane border border-accent/30 text-[9px] font-black uppercase tracking-[0.2em] text-accent hover:bg-accent hover:text-black transition-all shadow-sm active:scale-95 touch-manipulation"
                                        >
                                            {sit.label}
                                        </button>
                                    ))}
